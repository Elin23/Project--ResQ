...
const themeNames = ["COLORS", "FONTS", "FONT_SIZES", "SPACING", "RADIUS"];
...
for (const themeName of themeNames) {
  ...
  if (!objectMatch) {
    throw new Error(`Unable to read ${themeName} from src/theme/index.ts`);
  }

  const keys = new Set();
  for (const match of objectMatch[1].matchAll(/^\s*([A-Za-z_$][\w$]*)\s*:/gm)) {
    keys.add(match[1]);
  }
...
for (const filePath of files) {
  ...
  for (const themeName of themeNames) {
    ...
    const hasLocalDeclaration = new RegExp(
      `\\b(?:const|let|var|function|class)\\s+${escapeRegex(themeName)}\\b`,
    ).test(source);
...
console.log(
  `Theme usage check passed: ${files.length} files, no missing theme imports or properties.`,
);
