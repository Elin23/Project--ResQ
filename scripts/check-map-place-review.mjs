...
must("test-fixtures/repositories/inMemoryMapPlaceApplicationRepository.ts", 'current.status !== "pending"', "pending-only review transition");
must("test-fixtures/repositories/inMemoryMapPlaceApplicationRepository.ts", "A rejection reason is required", "required rejection reason");
must("test-fixtures/repositories/inMemoryMapPlaceApplicationRepository.ts", "Another active application already exists", "duplicate application prevention");
...
must("test-fixtures/repositories/inMemoryServicePlaceRepository.ts", 'item.status !== "archived"', "archived owner filtering");
must("test-fixtures/repositories/inMemoryServicePlaceRepository.ts", "A suspension reason is required", "suspension reason");
