...
for(const f of ['src/domain/service-places/mapPlaceValidation.ts','src/domain/service-places/mapPlaceChangeRequest.ts','src/domain/service-places/mapPlaceChangeRequestRepository.ts','test-fixtures/repositories/inMemoryMapPlaceChangeRequestRepository.ts','src/application/map-places/MapPlaceChangeReviewService.ts','src/features/map-places/components/OpeningHoursEditor.tsx','src/features/map-places/components/MapPlaceLocationPicker.native.tsx','src/features/map-places/screens/MapPlaceChangeRequestScreen.tsx','app/map-places/[id]/change-request.tsx']) if(!fs.existsSync(f))failures.push(`Missing ${f}`);
...
must('test-fixtures/repositories/inMemoryMapPlaceApplicationRepository.ts','validateMapPlaceDraft(current, { forSubmission: true })','repository submission validation');
...
must('src/features/map-places/screens/EditOwnedMapPlaceScreen.tsx','غير متاح حاليًا من تطبيق الجوال','unsupported sensitive-change disclosure');
mustNot('src/features/map-places/screens/EditOwnedMapPlaceScreen.tsx','mapPlaceChangeRequestRoute','broken sensitive-change CTA');
must('src/features/map-places/screens/MapPlaceChangeRequestScreen.tsx','لن يعرض التطبيق نجاحًا وهميًا','safe deep-link fallback');
mustNot('src/features/map-places/screens/MapPlaceChangeRequestScreen.tsx','repositories.mapPlaceChangeRequests.create','local-only sensitive change persistence');
must('test-fixtures/repositories/inMemoryMapPlaceChangeRequestRepository.ts',"x.placeId===input.placeId&&x.status==='pending'",'duplicate pending change prevention');
