...
if (!/COLORS\.navActive/.test(source)) failures.push("Active tab must be expressed with the navigation active tint");

if (failures.length) {
  console.error("Navbar Glass V2 check failed:\n" + failures.map((x) => `- ${x}`).join("\n"));
  process.exit(1);
}
console.log("Navbar Glass V2 check passed: no active box, floating overlay, frosted material, and tint-only active state enforced.");
