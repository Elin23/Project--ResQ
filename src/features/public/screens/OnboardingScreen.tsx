...
export default function OnboardingScreen() {
  ...
  const indicatorWidths = useMemo(() => ONBOARDING_ITEMS.map((_, index) => index === currentIndex ? 32 : 8), [currentIndex]);

  const animateToSlide = (nextIndex: number) => {
    if (isChanging || isCompleting || nextIndex === currentIndex || nextIndex < 0 || nextIndex >= ONBOARDING_ITEMS.length) return;
    setIsChanging(true);
    Animated.timing(opacity, { toValue: 0, duration: 90, easing: Easing.out(Easing.quad), useNativeDriver: true }).start(({ finished }) => {
      if (!finished) return setIsChanging(false);
      setCurrentIndex(nextIndex);
      translateY.setValue(8);
      Animated.parallel([
        Animated.timing(opacity, { toValue: 1, duration: 150, easing: Easing.out(Easing.quad), useNativeDriver: true }),
        Animated.timing(translateY, { toValue: 0, duration: 150, easing: Easing.out(Easing.quad), useNativeDriver: true }),
