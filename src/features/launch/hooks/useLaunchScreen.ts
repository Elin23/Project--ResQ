...
export function useLaunchScreen() {
  ...
  const { mode, principal, isReady } = useSession();
  ...
  useEffect(() => {
    ...
    const navigateFromSplash = async () => {
      ...
      if (onboardingCompleted) {
        router.replace(defaultRouteForPrincipal(principal));
        return;
      }
    ...
    Animated.timing(progress, {
      toValue: 1,
      duration: MOTION.launch.progress,
