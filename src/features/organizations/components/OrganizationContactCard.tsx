...
export default function OrganizationContactCard({ icon, label, value, tone, onPress }: Props) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${label}: ${value}`} onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
...
const styles=StyleSheet.create({ card:{flexDirection:"row",direction:"rtl",alignItems:"center",gap:SPACING.md,padding:SPACING.md,borderRadius:RADIUS.md,backgroundColor:COLORS.background,borderWidth:1,borderColor:COLORS.border,marginBottom:SPACING.md}, icon:{width:48,height:48,borderRadius:RADIUS.full,alignItems:"center",justifyContent:"center"}, text:{flex:1,alignItems:"stretch"}, pressed:{opacity:.75} });
